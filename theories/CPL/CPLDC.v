...
(* List of rules of the display calculus *)
...
Definition CPL_RIR := [atrefl; Topr; Botr; Negr; Conr; Disr; Impr].
Definition CPL_LIR := [atrefl; Topl; Botl; Negl; Conl; Disl; Impl].
(* Their defining properties. *)
Lemma CPL_RIR_ppty : forall r, r ∈ CPL_DC -> (r ∈ CPL_RIR <-> strIsFml (succ (conclRule r))).
Proof.
  intros r Hr. split.
  - intro HRIR. unfold CPL_RIR in HRIR. dest_in_list HRIR;
      rewrite <- HRIR; simpl; constructor.
  - intro H. match type of Hr with r ∈ ?DC => unfold DC in Hr end.
    dest_in_list Hr; rewrite <- Hr; auto_in;
      unfold strIsFml in H; rewrite <- Hr in H;
      rewrite (isVar_iff_isVar_cpt FS) in H;
      unfold isVar_cpt in H; simpl Var_dec in H;
      simpl in H; contradiction.
Qed.
Lemma CPL_LIR_ppty : forall r, r ∈ CPL_DC -> (r ∈ CPL_LIR <-> strIsFml (antec (conclRule r))).
Proof.
  intros r Hr. split.
  - intro HLIR. unfold CPL_LIR in HLIR. dest_in_list HLIR;
...
#[export] Instance CPL_INTRO_RULES : INTRO_RULES CPL_DC := {|
  RIR := CPL_RIR;
  LIR := CPL_LIR;
  RIR_ppty := CPL_RIR_ppty;
  LIR_ppty := CPL_LIR_ppty
|}.
...
(* CPL_DC' *)
  ...
  Proof.
    ...
    pose proof (@eq_refl _ (remove_rule CUT CPL_DC)) as Hrm.
    simpl remove_rule at 1 in Hrm.
    dest_in_list_eqdec rule_eq_dec Hr;
    rewrite Heq;
    try apply (alr_DerivRuleNC _ _);
    apply (derrnc_rules' _ Hrm); auto_in.
  Defined.
...
(*
  unfold RIR in HrL; unfold LIR in HrR.
*)
